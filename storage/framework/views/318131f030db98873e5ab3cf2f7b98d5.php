<?php $__env->startSection('title', $course->exists ? 'Sửa khóa học' : 'Tạo khóa học'); ?>
<?php $__env->startSection('page-title', $course->exists ? 'Sửa khóa học' : 'Tạo khóa học'); ?>
...
<div class="admin-form-page">
  <h2 class="admin-form-page__title"><?php echo e($course->exists ? 'Sửa khóa học' : 'Tạo khóa học'); ?></h2>

  <?php if(session('ok') || session('success')): ?>
    <div class="alert alert--success"><?php echo e(session('ok') ?? session('success')); ?></div>
  <?php endif; ?>
  ...
  <div class="admin-form-grid">
    <!-- Main Form -->
    <div>
      <form method="POST" action="<?php echo e($course->exists ? route('admin.courses.update',$course) : route('admin.courses.store')); ?>" class="admin-form" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        <?php if($course->exists): ?> <?php echo method_field('PUT'); ?> <?php endif; ?>

        <div class="admin-form__field">
          <label class="admin-form__label">Tiêu đề *</label>
          <input name="title" value="<?php echo e(old('title',$course->title)); ?>" class="admin-form__input" required>
        </div>

        <div class="admin-form__field">
          <label class="admin-form__label">Slug *</label>
          <input name="slug" value="<?php echo e(old('slug',$course->slug)); ?>" class="admin-form__input" required>
        </div>
        ...
        <div class="admin-form__field">
          <label class="admin-form__label">Giảng viên</label>
          <select name="instructor_id" class="admin-form__select">
          ...
          <small class="admin-help-text">Chọn giảng viên phụ trách khóa học này</small>
        </div>
        ...
        <div class="admin-form__field">
          <div class="admin-form__grid admin-form__grid--2">
            <div>
              ...
              <input type="number" name="price" value="<?php echo e(old('price',$course->price)); ?>" class="admin-form__input" min="0" step="1000">
            </div>
            <div>
              <label class="admin-form__label">Giá so sánh (₫)</label>
              <input type="number" name="compare_at_price" value="<?php echo e(old('compare_at_price',$course->compare_at_price)); ?>" class="admin-form__input" min="0" step="1000">
            </div>
          </div>
        </div>
          ...
          <div class="admin-form__grid admin-form__grid--3">
            <div>
              ...
              <select name="status" class="admin-form__select" required>
                ...
                <option value="hidden" <?php echo e(old('status',$course->status)=='hidden'?'selected':''); ?>>Hidden</option>
            ...
            <div>
              ...
              <select name="level" class="admin-form__select">
                ...
                <option value="beginner" <?php echo e(old('level',$course->level)=='beginner'?'selected':''); ?>>Cơ bản</option>
                <option value="intermediate" <?php echo e(old('level',$course->level)=='intermediate'?'selected':''); ?>>Trung bình</option>
                <option value="advanced" <?php echo e(old('level',$course->level)=='advanced'?'selected':''); ?>>Nâng cao</option>
              </select>
            </div>
            <div>
              <label class="admin-form__label">Ngôn ngữ</label>
              <input name="language" value="<?php echo e(old('language',$course->language ?? 'Vietnamese')); ?>" class="admin-form__input">
            </div>
          </div>
        </div>
        ...
        <div class="admin-form__field">
          ...
          <textarea name="short_description" class="admin-form__textarea" rows="3"><?php echo e(old('short_description',$course->short_description)); ?></textarea>
        </div>
        ...
        <div class="admin-form__field">
          ...
          <?php if($course->thumbnail_path): ?>
            <div style="margin-bottom:8px">
              <img src="<?php echo e($course->thumbnail_url); ?>" alt="Thumbnail" style="max-width:220px;border-radius:6px;border:1px solid #e5e7eb">
            </div>
          <?php endif; ?>
        ...
        <div class="admin-form__field">
          ...
          <textarea id="editor" name="description_html" rows="10"><?php echo e(old('description_html',$course->description_html)); ?></textarea>
        </div>

        <div class="admin-form__actions">
          <button type="submit" class="btn btn--primary"><?php echo e($course->exists ? 'Lưu thay đổi' : 'Tạo mới'); ?></button>
          <?php if($course->exists): ?>
            <a class="btn btn--outline" href="<?php echo e(route('courses.show',$course->slug)); ?>" target="_blank">Xem trang</a>
          <?php endif; ?>
        </div>
      </form>
      ...
      <?php if($course->exists): ?>
        <div class="admin-content-management">
          ...
          <?php $__currentLoopData = $course->sections()->with('lessons')->orderBy('position')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="admin-section">
              <div class="admin-section__header">
                <div class="admin-flex--1">
                  <strong class="admin-section__title"><?php echo e($section->title); ?></strong>
                  <span class="admin-section__meta">(<?php echo e($section->lessons->count()); ?> bài học)</span>
                </div>
                <div class="admin-section__actions">
                  <button onclick="editSection(<?php echo e($section->id); ?>, '<?php echo e($section->title); ?>', <?php echo e($section->position); ?>)" 
                          class="btn btn--outline btn--sm" type="button">
                    Sửa
                  </button>
                  <form action="<?php echo e(route('admin.sections.destroy', $section)); ?>" method="POST" class="admin-actions-container--inline">
                    <?php echo csrf_field(); ?>
              ...
              <div>
                <?php $__currentLoopData = $section->lessons()->orderBy('position')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lesson): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <div class="admin-lesson">
                    <div class="admin-lesson__content">
                      <div class="admin-lesson__title"><?php echo e($lesson->title); ?></div>
                      <div class="admin-lesson__meta">
                        <?php echo e(\App\Helpers\VideoHelper::formatDuration($lesson->duration)); ?>
                    ...
                    <div class="admin-lesson__actions">
                      <button onclick="editLesson(<?php echo e($lesson->id); ?>, <?php echo e($section->id); ?>, '<?php echo e(addslashes($lesson->title)); ?>', '<?php echo e($lesson->video_path ?? ''); ?>', '<?php echo e($lesson->video_url ?? ''); ?>', '<?php echo e($lesson->attachment_path ?? ''); ?>', <?php echo e($lesson->duration ?? 0); ?>, <?php echo e($lesson->is_preview ? 'true' : 'false'); ?>, <?php echo e($lesson->position ?? 0); ?>)" 
                              class="btn btn--outline btn--sm" type="button">
                        Sửa
                      </button>
                      <form action="<?php echo e(route('admin.lessons.destroy', $lesson)); ?>" method="POST" class="admin-actions-container--inline">
                        <?php echo csrf_field(); ?>
                        <?php echo method_field('DELETE'); ?>
                ...
                <div class="admin-lesson__add">
                  <button onclick="showAddLessonForm(<?php echo e($section->id); ?>)" 
          ...
          <form action="<?php echo e(route('admin.sections.store', $course)); ?>" method="POST" class="admin-section__add-form">
            <?php echo csrf_field(); ?>
      ...
      <div class="admin-form-sidebar__card">
        ...
        <?php if($course->exists): ?>
          <div class="admin-form-sidebar__item">
            ...
            <p class="admin-form-sidebar__item-value"><?php echo e($course->id); ?></p>
          </div>
          <div class="admin-form-sidebar__item">
            <p class="admin-form-sidebar__item-label">Ngày tạo</p>
            <p class="admin-form-sidebar__item-value"><?php echo e($course->created_at->format('d/m/Y H:i')); ?></p>
          </div>
          <div class="admin-form-sidebar__item">
            <p class="admin-form-sidebar__item-label">Học viên</p>
            <p class="admin-form-sidebar__item-value"><?php echo e($course->enrolled_students); ?></p>
          </div>
          <div class="admin-form-sidebar__item">
            <p class="admin-form-sidebar__item-label">Đánh giá</p>
            <p class="admin-form-sidebar__item-value"><?php echo e(number_format($course->rating, 1)); ?> ⭐ (<?php echo e($course->rating_count); ?>)</p>
          </div>
        <?php endif; ?>
      </div>
    </aside>
...
function fetchYouTubeDuration(videoUrl) {
  ...
  // Call API
  fetch('<?php echo e(route("admin.api.youtube.duration")); ?>', {
    method: 'POST',
    headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\elearning\resources\views/admin/courses/form.blade.php ENDPATH**/ ?>