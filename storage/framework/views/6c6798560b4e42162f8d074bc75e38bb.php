...
<link rel="stylesheet" href="<?php echo e(asset('css/pages/student-lesson.css')); ?>">
<?php $__env->stopPush(); ?>
...
<nav class="breadcrumb" style="margin-bottom: var(--spacing-lg);">
  ...
  <a href="<?php echo e(route('student.learn', $course->slug)); ?>"><?php echo e($course->title); ?></a>
  <span style="margin: 0 var(--spacing-sm);">/</span>
...
<section class="lesson-page">
  <div class="lesson-grid">
    ...
    <aside class="lesson-sidebar">
      ...
      <?php echo $__env->make('student.partials.sidebar', ['currentLessonId' => $lesson->id], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    </aside>
    ...
    <main class="lesson-main">
      ...
      <?php if($isLocked): ?>
        <div class="lesson-locked">
          ...
          <p class="lesson-locked__message"><?php echo e($lockedReason); ?></p>
          <?php if($previousLesson): ?>
            <a href="<?php echo e(route('student.lesson', ['course' => $course->slug, 'lesson' => $previousLesson->id])); ?>" 
      ...
      <div class="lesson-video <?php echo e($isLocked ? 'lesson-video--locked' : ''); ?>">
        <?php
          $videoEmbedUrl = null;
          $directVideoUrl = null;
          $useIframe = false;
        ...
        <?php if($cloudinaryDirectUrl): ?>
          
          <video id="lessonVideo" controls controlsList="nodownload" preload="metadata" style="width: 100%; height: 100%;">
            <source src="<?php echo e($cloudinaryDirectUrl); ?>" type="video/mp4">
            <source src="<?php echo e($cloudinaryDirectUrl); ?>" type="video/webm">
            <div class="lesson-video__fallback">
              <p>Trình duyệt của bạn không hỗ trợ video. Vui lòng tải video về để xem.</p>
            </div>
          </video>
        <?php elseif($useIframe && $videoEmbedUrl): ?>
          
          <div id="videoContainer" style="position: relative; width: 100%; height: 100%; min-height: 400px;">
            <iframe 
              id="lessonVideoIframe" 
              src="<?php echo e($videoEmbedUrl); ?>?enablejsapi=1&origin=<?php echo e(urlencode(request()->getSchemeAndHttpHost())); ?>&rel=0&modestbranding=1&controls=1&showinfo=0&iv_load_policy=3&cc_load_policy=0&fs=1&playsinline=1" 
              frameborder="0" 
          ...
          <script>
            ...
            console.log('YouTube/Vimeo iframe created', {
              embedUrl: '<?php echo e($videoEmbedUrl); ?>',
              useIframe: <?php echo e($useIframe ? 'true' : 'false'); ?>,
              videoUrl: '<?php echo e($lesson->video_url); ?>',
        ...
        <?php elseif($directVideoUrl): ?>
          <video id="lessonVideo" controls controlsList="nodownload" preload="metadata">
            <source src="<?php echo e($directVideoUrl); ?>" type="video/mp4">
            <source src="<?php echo e($directVideoUrl); ?>" type="video/webm">
            <source src="<?php echo e($directVideoUrl); ?>" type="video/ogg">
            <div class="lesson-video__fallback">
              <p>
                Trình duyệt của bạn không hỗ trợ video. 
                <?php if(\App\Helpers\VideoHelper::isGoogleDrive($lesson->video_url ?? '')): ?>
                  <br><small>Nếu video không hiển thị, vui lòng đảm bảo file Google Drive được chia sẻ công khai (Anyone with the link)</small>
                <?php endif; ?>
              </p>
            </div>
          </video>
        <?php elseif($lesson->hls_path): ?>
          ...
          <script>
            document.addEventListener('DOMContentLoaded', function() {
              ...
              const hlsUrl = '<?php echo e(Storage::url($lesson->hls_path)); ?>';
      ...
      <div class="lesson-info card">
        <div class="lesson-info__header">
          <div>
            <h1 class="lesson-info__title"><?php echo e($lesson->title); ?></h1>
            <p class="lesson-info__meta">
              <?php echo e($lesson->section->title); ?> • <?php echo e(\App\Helpers\VideoHelper::formatDuration($lesson->duration)); ?>
          ...
          <?php elseif(!isset($progress[$lesson->id])): ?>
        ...
        <?php if($lesson->attachment_path): ?>
          <div class="lesson-attachment">
            <a href="<?php echo e(Storage::url($lesson->attachment_path)); ?>" 
      ...
      <div class="lesson-navigation">
        <?php if($previousLesson): ?>
          <a href="<?php echo e(route('student.lesson', ['course' => $course->slug, 'lesson' => $previousLesson->id])); ?>" 
            ...
            ← Bài trước: <?php echo e($previousLesson->title); ?>
        ...
        <?php if($nextLesson): ?>
          <a href="<?php echo e(route('student.lesson', ['course' => $course->slug, 'lesson' => $nextLesson->id])); ?>" 
             class="lesson-navigation__link lesson-navigation__link--next">
            Bài tiếp theo: <?php echo e($nextLesson->title); ?> →
          </a>
        <?php else: ?>
          <div class="lesson-navigation__spacer"></div>
        <?php endif; ?>
...
function completeLesson() {
  ...
  fetch('<?php echo e(route("student.complete-lesson", $lesson->id)); ?>', {
    method: 'POST',
    headers: {
  ...
  .then(data => {
    if (data.success) {
      ...
      fetch('<?php echo e(route("admin.api.courses.sidebar", $course->slug)); ?>?lesson=<?php echo e($lesson->id); ?>', {
        headers: {
      ...
      .then(data => {
        if (data.html) {
          ...
          if (oldSidebar) {
            ...
            const currentLessonId = <?php echo e($lesson->id); ?>;
            ...
            if (newSections && oldSections) {
              ...
              oldSections.querySelectorAll('.lesson-sidebar__section').forEach(section => {
                ...
                const currentLink = section.querySelector(`a[href*="/lessons/<?php echo e($lesson->id); ?>"]`);
                if (currentLink) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const isLessonAlreadyCompleted = <?php echo e($isLessonCompleted ? 'true' : 'false'); ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\elearning\resources\views/student/lesson.blade.php ENDPATH**/ ?>